...
func _test_table_tennis_mapping() -> void:
	...
	_expect(centered_return.z < -4.5 and centered_return.y > 1.0, "hand contact should return the ball toward the computer")
	_expect(TableTennisRulesForTest.hand_return_velocity(0.4, 0.0).x > 0.0, "off-center hand contact should steer the return")
	_expect(TableTennisRulesForTest.computer_return_velocity(0.0, 0.0).z > 4.0, "computer should return the ball toward the player")
